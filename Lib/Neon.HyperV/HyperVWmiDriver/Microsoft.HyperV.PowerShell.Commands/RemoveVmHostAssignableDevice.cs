...
[Cmdlet("Remove", "VMHostAssignableDevice", DefaultParameterSetName = "Path", SupportsShouldProcess = true)]
internal sealed class RemoveVmHostAssignableDevice : VirtualizationCmdlet<Tuple<VMPciExpressResourcePool, IEnumerable<VMHostAssignableDevice>>>, ISupportsForce
{
	...
	internal override IList<Tuple<VMPciExpressResourcePool, IEnumerable<VMHostAssignableDevice>>> EnumerateOperands(IOperationWatcher operationWatcher)
	{
		...
		{
			devices = inputs.SelectManyWithLogging((Server server) => Microsoft.HyperV.PowerShell.VMHostAssignableDevice.FindHostAssignableDevices(server, InstancePath, LocationPath), operationWatcher).ToList();
		}
		else
		{
	...
	internal override void ProcessOneOperand(Tuple<VMPciExpressResourcePool, IEnumerable<VMHostAssignableDevice>> operand, IOperationWatcher operationWatcher)
	{
		...
		if (operationWatcher.ShouldProcess(string.Format(CultureInfo.CurrentCulture, CmdletResources.ShouldProcess_RemoveVMHostAssignableDevice, item.Name)) && operationWatcher.ShouldContinue(string.Format(CultureInfo.CurrentCulture, CmdletResources.ShouldContinue_RemoveVMHostAssignableDevice, item.Name)))
		{
			item.RemovePciExpressDevices(item2, operationWatcher);
		}
	}
}
