...
internal sealed class NewVM : VirtualizationCreationCmdlet<VirtualMachine>, ISupportsAsJob, ISupportsForce
{
	...
	protected override void ValidateParameters()
	{
		...
		{
			string extension = System.IO.Path.GetExtension(NewVHDPath);
		...
		if (Generation == 2 && BootDevice.HasValue)
		{
			...
			switch (value)
			{
			case Microsoft.HyperV.PowerShell.BootDevice.Floppy:
				text = CmdletErrorMessages.Generation2_NoFloppySupport;
				break;
			case Microsoft.HyperV.PowerShell.BootDevice.IDE:
				text = CmdletErrorMessages.Generation2_NoIDESupport;
				break;
			case Microsoft.HyperV.PowerShell.BootDevice.LegacyNetworkAdapter:
	...
	private void ConfigureVirtualMachine(VirtualMachine vm, IOperationWatcher operationWatcher)
	{
		...
		else if (BootDevice == Microsoft.HyperV.PowerShell.BootDevice.CD)
		{
	...
	private string ConfigureHardDrive(VirtualMachine vm, IOperationWatcher operationWatcher)
	{
		...
		{
			string path = ((!string.IsNullOrEmpty(System.IO.Path.GetDirectoryName(VHDPath))) ? PathUtility.GetFullPath(VHDPath, base.CurrentFileSystemLocation) : System.IO.Path.Combine(VirtualizationObjectLocator.GetVMHost(vm.Server).VirtualHardDiskPath, VHDPath));
			AddVhdToVm(vm, path, operationWatcher);
		}
		else if (CurrentParameterSetIs("New VHD"))
		{
			string text = ((!string.IsNullOrEmpty(System.IO.Path.GetDirectoryName(NewVHDPath))) ? PathUtility.GetFullPath(NewVHDPath, base.CurrentFileSystemLocation) : (string.IsNullOrEmpty(Path) ? System.IO.Path.Combine(VirtualizationObjectLocator.GetVMHost(vm.Server).VirtualHardDiskPath, NewVHDPath) : System.IO.Path.Combine(System.IO.Path.Combine(Path, "Virtual Hard Disks"), NewVHDPath)));
	...
	private void ConfigureVMNetwork(VirtualMachine vm, IOperationWatcher operationWatcher)
	{
		...
		if (Generation == 1 && BootDevice.HasValue)
		{
			...
			{
			case Microsoft.HyperV.PowerShell.BootDevice.LegacyNetworkAdapter:
			case Microsoft.HyperV.PowerShell.BootDevice.NetworkAdapter:
				isEmulated = true;
				break;
			}
		}
