...
[Cmdlet("Enable", "VMRemoteFXPhysicalVideoAdapter", SupportsShouldProcess = true, DefaultParameterSetName = "GPUByName")]
[OutputType(new Type[] { typeof(VMRemoteFXPhysicalVideoAdapter) })]
internal sealed class EnableVMRemoteFXPhysicalVideoAdapter : VirtualizationCmdlet<VMRemoteFXPhysicalVideoAdapter>, ISupportsPassthrough
{
	...
	{
		if (operationWatcher.ShouldProcess(string.Format(CultureInfo.CurrentCulture, CmdletResources.ShouldProcess_EnableVMRemoteFXPhysicalVideoAdapter, operand.Name, operand.Server)))
		{
			operand.SetGPUEnabledForVirtualizationState(enabled: true, operationWatcher);
