...
[Cmdlet("Connect", "VMSan", DefaultParameterSetName = "StringWwn", SupportsShouldProcess = true)]
[OutputType(new Type[] { typeof(VMSan) })]
internal sealed class ConnectVMSan : VirtualizationCmdlet<VMSan>, IVMSanCmdlet, IParameterSet, ISupportsPassthrough
{
    ...
    {
        if (operationWatcher.ShouldProcess(string.Format(CultureInfo.CurrentCulture, CmdletResources.ShouldProcess_ConnectVMSan, operand.Name)))
        {
            ParameterResolvers.GetHbaNames(this, out var nodeWwns, out var portWwns);
            operand.ConnectHbas(nodeWwns, portWwns, operationWatcher);
