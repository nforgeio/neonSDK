...
[Cmdlet("Disconnect", "VMSan", DefaultParameterSetName = "StringWwn", SupportsShouldProcess = true)]
[OutputType(new Type[] { typeof(VMSan) })]
internal sealed class DisconnectVMSan : VirtualizationCmdlet<VMSan>, IVMSanCmdlet, IParameterSet, ISupportsPassthrough
{
    ...
    [Parameter(Mandatory = true, ValueFromPipeline = true, ParameterSetName = "HbaObject")]
    [ValidateNotNullOrEmpty]
    ...
    {
        if (operationWatcher.ShouldProcess(string.Format(CultureInfo.CurrentCulture, CmdletResources.ShouldProcess_DisconnectVMSan, operand.Name)))
        {
            ParameterResolvers.GetHbaNames(this, out var nodeWwns, out var portWwns);
            operand.DisconnectHbas(nodeWwns, portWwns, operationWatcher);
