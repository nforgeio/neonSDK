...
[Cmdlet("Measure", "VMReplication", DefaultParameterSetName = "VMName")]
[OutputType(new Type[] { typeof(VMReplicationHealth) })]
internal sealed class MeasureVMReplication : VirtualizationCmdlet<VirtualMachine>, IVmByVMNameCmdlet, IVirtualMachineCmdlet, IServerParameters, IParameterSet, IVmByObjectCmdlet
{
    ...
    protected override void ValidateParameters()
    {
        ...
        {
            throw ExceptionHelper.CreateInvalidStateException(string.Format(CultureInfo.CurrentCulture, CmdletErrorMessages.VMReplication_NoRecord_If_State_Disabled, "Measure-VMReplication"), null, null);
        }
    }
    ...
    private void ProcessVirtualMachineForReplicationType(VirtualMachine virtualMachine, VMReplicationRelationshipType replicationRelationshipType, bool isFirstCheck, IOperationWatcher operationWatcher)
    {
        ...
        if (vMReplication == null || !vMReplication.IsEnabled)
        {
            ...
            {
                ExceptionHelper.DisplayErrorOnException(ExceptionHelper.CreateObjectNotFoundException(string.Format(CultureInfo.CurrentCulture, CmdletErrorMessages.VMReplication_NotEnabled, virtualMachine.Name), null), operationWatcher);
            }
        }
        ...
        {
            operationWatcher.WriteObject(new VMReplicationHealth(vMReplication));
        }
    }
}
