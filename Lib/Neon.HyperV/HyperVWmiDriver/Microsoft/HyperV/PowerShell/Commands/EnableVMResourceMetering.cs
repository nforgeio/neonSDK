...
[Cmdlet("Enable", "VMResourceMetering", DefaultParameterSetName = "VMName")]
internal sealed class EnableVMResourceMetering : VirtualizationCmdlet<object>, IVMObjectOrVMNameCmdlet, IVmByObjectCmdlet, IVirtualMachineCmdlet, IServerParameters, IParameterSet, IVmByVMNameCmdlet
{
    ...
    {
        if (operand is VirtualMachine vm)
        {
            MetricUtilities.ConfigureMeteringPortAcls(vm, operationWatcher);
        }
        MetricUtilities.ChangeMetricServiceState(MetricUtilities.MetricServiceState.Enabled, (IMeasurable)operand);
    }
}
