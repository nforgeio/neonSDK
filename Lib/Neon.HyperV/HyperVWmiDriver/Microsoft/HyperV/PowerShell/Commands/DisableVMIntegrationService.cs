...
[Cmdlet("Disable", "VMIntegrationService", SupportsShouldProcess = true, DefaultParameterSetName = "VMName")]
[OutputType(new Type[] { typeof(VMIntegrationComponent) })]
internal sealed class DisableVMIntegrationService : VirtualizationCmdlet<VMIntegrationComponent>, IVMObjectOrVMNameCmdlet, IVmByObjectCmdlet, IVirtualMachineCmdlet, IServerParameters, IParameterSet, IVmByVMNameCmdlet, ISupportsPassthrough
{
    ...
    {
        if (operationWatcher.ShouldProcess(string.Format(CultureInfo.CurrentCulture, CmdletResources.ShouldProcess_DisableVMIntegrationService, integrationComponent.Name, integrationComponent.VMName)))
        {
            integrationComponent.Enabled = false;
