...
internal sealed class VMReplicationServer : VirtualizationObject, IUpdatable
{
	...
	public RecoveryAuthenticationType AllowedAuthenticationType
	{
		...
		internal set
		{
			...
			{
				m_ServiceSetting.GetData(UpdatePolicy.None).AllowedAuthenticationType = (Microsoft.Virtualization.Client.Management.RecoveryAuthenticationType)value;
			}
		}
	}
	...
	internal void StartReplication(VirtualMachine vm, InitialReplicationType initialReplicationType, string initialReplicationExportLocation, DateTime startTime, IOperationWatcher operationWatcher)
	{
		IVMComputerSystem vmObject = vm.GetComputerSystemAs<IVMComputerSystem>(UpdatePolicy.EnsureUpdated);
		IReplicationService service = m_Service.GetData(UpdatePolicy.None);
		operationWatcher.PerformOperation(() => service.BeginStartReplication(vmObject, (Microsoft.Virtualization.Client.Management.InitialReplicationType)initialReplicationType, initialReplicationExportLocation, startTime), service.EndStartReplication, TaskDescriptions.Task_StartReplication, null);
		vmObject.InvalidatePropertyCache();
		vmObject.InvalidateAssociationCache();
	}
	...
	{
		IReplicationService service = m_Service.GetData(UpdatePolicy.None);
		operationWatcher.PerformOperation(() => service.BeginTestReplicationConnection(recoveryConnectionPoint, recoveryServerPortNumber, (Microsoft.Virtualization.Client.Management.RecoveryAuthenticationType)authenticationType, certificateThumbPrint, bypassProxyServer), service.EndTestReplicationConnection, TaskDescriptions.Task_TestReplicationConnection, null);
	}
