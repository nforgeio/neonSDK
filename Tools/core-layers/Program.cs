...
namespace CoreLayers
{
    ...
    /// the <b>__dep</b> folder will include all of the assemblies and packages
    ...
    /// </para>
    /// <note>
    /// This tool also removes any codedoc XML files from these too folders to reduce
...
usage: core-layers ASSEMBLY PATH";
        ...
        /// <summary>
        /// Examines a file to determine whether it is a C# CodeDoc XML file.
        /// </summary>
        /// <param name="path">Path to the file being examined.</param>
        /// <returns><c>true</c> for CodeDoc files.</returns>
        public static bool IsCodeDoc(string path)
        {
            if (!File.Exists(path) || !Path.GetExtension(path).Equals(".xml", StringComparison.InvariantCultureIgnoreCase))
            {
                return false;
            }

            // Read the XML file and look for a document that starts like:
            //
            //      <?xml version="1.0"?>
            //      <doc>
            //          <assembly>
            ...
            try
            {
                using (var reader = new StreamReader(path))
                {
                    ...
                    if (xDoc.Root?.Name.LocalName != "doc")
                    {
                        return false;
                    }

                    var assemblyElement = xDoc.Root.Elements().FirstOrDefault();
                    ...
                    if (assemblyElement.Name.LocalName != "assembly")
                    {
                        return false;
                    }

                    var assemblyNameElement = assemblyElement.Elements().FirstOrDefault();
                    ...
                    return assemblyNameElement.Name.LocalName == "name";
                }
            }
            catch (IOException)
            {
